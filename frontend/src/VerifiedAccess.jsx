...
export function IntegrationSettings({ api, user }) {
  ...
  async function action(name, body = {}) {
    ...
    try {
      ...
      setMessage(name === "check-coexistence" ? (!result.coexistence_confirmed ? "Coexistence is not confirmed. Do not migrate or disconnect the number." : !result.phone_connected ? "Coexistence is enabled, but Meta does not report the number as connected." : !result.messaging_permission_granted ? "The number is connected, but the server token lacks WhatsApp messaging permission required for incoming messages. Delivery is not ready." : "Meta confirms coexistence, a connected number and messaging permission. Verify a fresh ad enquiry below; configuration alone does not prove delivery.") : name === "map-ad" ? "Ad-to-offering mapping saved." : `Attribution retried for ${result.attempted} ads.`);
      await refresh();
    } catch (error) { setMessage(error.message); } finally { setBusy(false); }
  }
  ...
  return <section className="card settings-card integration-panel"><h2>WhatsApp → lead inbox</h2>
    <p className="settings-copy">Only a message carrying a Meta ad referral can start an automatic lead. Ordinary chats and opening a chat alone do not create leads. One campaign + original sender phone is one lead, across all ads in that campaign.</p>
    ...
    {!state ? <p>Connection details are not available yet. Apply the integration migration and configure server access.</p> : <>
      <div className="settings-list">{Object.entries(state.configured).map(([key, ready]) => <div key={key}><span>{({ signing: "Webhook signature secret", verification: "Webhook verification token", ads_read: "Ads read credential", whatsapp_read: "WhatsApp credential" })[key]}</span><strong>{ready ? "Present—not delivery verified" : "Not configured"}</strong></div>)}</div>
      ...
      <p className="integration-notice" role="status">{state.delivery?.last_stored_at ? `Last message saved to a lead: ${new Date(state.delivery.last_stored_at).toLocaleString()}. ${state.delivery.recent_delivery_observed ? "A delivery was observed in the past 24 hours." : "No successful lead delivery observed in the past 24 hours—check the Business app for enquiries missing here."}` : "No message has been saved to a lead yet."} This does not verify that every enquiry has arrived.</p>
      <p>Unresolved ad lookups: {state.pending.length}{state.pending.length === 20 ? "+" : ""}</p>
      {state.waiting?.total > 0 && <p className="integration-notice" role="status">{state.waiting.total}{state.waiting.capped ? "+" : ""} ad messages safely queued before lead creation; {state.waiting.missing_phone} need a sender phone supplied by Meta. Retry campaign lookups for unresolved campaign metadata. No campaign or phone is guessed.</p>}
      {(state.errors || []).map((item, index) => <p role="alert" key={index}>Incoming message error {item.error_code} at {new Date(item.received_at).toLocaleString()}. Check the original enquiry in the Business app; it may need manual reconciliation.</p>)}
      <div className="integration-actions"><button className="secondary-button" disabled={busy} onClick={() => action("check-coexistence")}>Check Meta connection (read-only)</button><button className="secondary-button" disabled={busy} onClick={() => action("retry-attribution")}>Retry campaign lookups</button><button className="secondary-button" disabled={busy} onClick={refresh}>Refresh status</button></div>
