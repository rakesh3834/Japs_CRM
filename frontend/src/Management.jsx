...
export function ContactsManager({ contacts, api, user, onSaved, onEditLead }) {
  ...
  const visible = contacts.filter((c) => [c.name, c.phone, c.email, c.type, c.notes].join(" ").toLowerCase().includes(query.trim().toLowerCase()));
  ...
  return <><Heading title="Contacts" description="Contact details, linked enquiries and their original ad attribution." action={canManage(user) ? () => setEditing({}) : null} label="Add contact" />
    <div className="contacts-toolbar"><label className="module-search"><span>Search contacts</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Name, phone, email or notes" /></label><span>{visible.length} contact{visible.length === 1 ? "" : "s"}</span></div>
    <section className="card contacts-directory" aria-label="Contact directory"><div className="contacts-table-wrap"><table className="contacts-data-table"><colgroup><col className="contacts-col-name" /><col className="contacts-col-phone" /><col className="contacts-col-email" /><col className="contacts-col-type" /><col className="contacts-col-added" /><col className="contacts-col-actions" /></colgroup><thead><tr><th>Contact name</th><th>Phone number</th><th>Email</th><th>Type</th><th>Added</th><th><span className="visually-hidden">Actions</span></th></tr></thead><tbody>{visible.map((contact) => <tr key={contact.id} className={selected === contact.id ? "selected-contact-row" : ""}>
      <td><button className="contact-name-link" onClick={() => setSelected(contact.id)}>{contact.name || "Unnamed contact"}</button>{contact.notes && <small className="contact-note-preview">{contact.notes}</small>}</td><td>{contact.phone ? <a className="leads-phone-link" href={`tel:${contact.phone}`}>{contact.phone}</a> : "Not supplied"}</td><td>{contact.email || "Not supplied"}</td><td><span className="contact-type-pill">{contact.type || "Customer"}</span></td><td>{contact.created_at ? when(contact.created_at) : "Not recorded"}</td><td className="contact-row-actions"><button className="secondary-button" onClick={() => setSelected(contact.id)}>View</button>{canManage(user) && <button className="secondary-button" onClick={() => setEditing(contact)}>Edit</button>}</td>
    </tr>)}</tbody></table></div>{!visible.length && <Empty>No matching contacts.</Empty>}</section>
    <section className="card contact-detail" aria-label="Contact details">{error && <p role="alert" className="integration-notice">{error}</p>}{!detail ? <Empty>{selected ? "Loading contact…" : "Select a contact to review and manage their enquiries."}</Empty> : <><header className="card-header"><div><h2>{detail.item.name}</h2><p>{detail.item.type}</p></div>{canManage(user) && <button className="secondary-button" onClick={() => setEditing(detail.item)}>Edit contact</button>}</header><dl className="record-facts inset">{[["Phone", detail.item.phone], ["Email", detail.item.email], ["Contact notes", detail.item.notes], ["Added", when(detail.item.created_at)]].map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value || "Not supplied"}</dd></div>)}</dl><h3 className="inset">Linked enquiries</h3>{detail.leads.map((lead) => <article className="contact-enquiry" key={lead.uuid}><header><strong>{lead.destination} · {lead.status}</strong><button className="secondary-button" onClick={() => onEditLead(lead)}>{canManage(user) ? "Review / edit lead" : "View lead"}</button></header><p>{lead.dates} · {lead.travelers} · Budget: {lead.budget == null ? "Not supplied" : money(lead.budget)}</p><p>Next: {lead.next} · Owner: {lead.owner}</p>{lead.notes && <p className="preserve-text">{lead.notes}</p>}{lead.lost_reason && <p>Lost reason: {lead.lost_reason}</p>}<LeadFacts lead={lead} /></article>)}{!detail.leads.length && <Empty>No linked enquiries yet.</Empty>}{detail.next_offset !== null && <button className="secondary-button" disabled={busy} onClick={more}>Load older enquiries</button>}</>}</section>
