...
import { operationsRoute, readTasks } from "./operations.js";
...
import { PRIMARY_LEAD_STATUSES, normalizeLeadStatus } from "../shared/lead-statuses.js";
...
async function handleApi(request, env, url, ctx) {
  ...
  try {
    ...
    const operations = await operationsRoute(request, env, user, url);
    ...
    if (url.pathname === "/api/dashboard" && request.method === "GET") {
      const [leadRows, trips, payments, tasks] = await Promise.all([allRows(env, "crm_lead_inbox", { select: "*", organization_id: `eq.${user.organization_id}`, deleted_at: "is.null", order: "created_at.desc,id.desc" }), remoteTrips(env, user), remotePayments(env, user), readTasks(env, user)]);
      ...
      return respond({ ok: true, source: "supabase", workspace: { name: "Japs Travels", brand: "Japs_CRM", currency: "INR", tax_rate: 5 }, stats: { open_enquiries: leads.filter((item) => !["Won", "Lost"].includes(item.status)).length, active_trips: trips.filter((item) => item.stage !== "Close" && item.health !== "Closed").length, customer_due: incomingDue, margin_at_risk: trips.filter((item) => item.health === "At risk").length }, analytics: dashboardAnalytics(leads, tasks), leads: leads.filter((item) => PRIMARY_LEAD_STATUSES.includes(normalizeLeadStatus(item.status))).slice(0, 5), trips, payments, viewer: user }, 200, cors);
    }
    ...
    if (url.pathname === "/api/leads" && request.method === "POST") {
      ...
      return respond({ ok: true, synced: true, item: { id: code, name: body.name, phone: body.phone, email: body.email || "", source: body.source || "Manual", destination: body.destination, travelers: `${Number(body.travelers || 2)} travelers`, status: "Yet to contact", next: "Qualify this lead", value: 0 } }, 201, cors);
    }
...
function readableDates(start, end) { if (!start) return "Dates flexible"; const format = (value) => new Date(`${value}T00:00:00Z`).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }); return `${format(start)}${end ? ` – ${format(end)}` : ""}`; }
